			...
			{allImages.length > 0 && (
				<div className="mt-2">
					...
					<div
						...
						style={{ width: "100%", aspectRatio: "16/9", background: "var(--bs-secondary-bg)", userSelect: "none" }}
						...
						{allImages.length > 1 && (
							<>
								...
								<span
									...
									style={{ fontSize: 11 }}
								>{clampedActive + 1} / {allImages.length}</span>
							</>
						)}
					</div>
					...
					{allImages.length > 1 && (
						<div className="d-flex gap-2 mt-2 pb-1" style={{ overflowX: "auto", scrollbarWidth: "thin" }}>
							{allImages.map((img, i) => (
								...
								>
									<img src={img.src} alt={`thumb-${i}`} draggable={false}
