...
const EventCreationForm = () => {
	...
	const handleSaveDraft = async (e: React.MouseEvent) => {
		...
		try {
			...
			const payload = {
				...buildPayload(),
				date:     formData.date     || todayStr,       // required by schema
				location: formData.location || "TBD",          // required by schema
				capacity: Number(formData.capacity) || 1,      // required by schema
			};
			let savedId: string;
				...
				<div className="d-flex flex-wrap gap-2 mt-2">
					{imagePreviews.map((src, i) => (
						<div key={i} className="position-relative" style={{ width: 80, height: 80 }}>
							...
							<button type="button" onClick={() => removeNewImage(i)} className="btn btn-danger btn-sm position-absolute top-0 end-0 p-0 lh-1" style={{ width: 20, height: 20, fontSize: 12 }}>×</button>
						</div>
					))}
				</div>
			)}
				...
				<div className="d-flex flex-wrap gap-2 mt-2">
					{existingUrls.map((url, i) => (
						<div key={i} className="position-relative" style={{ width: 80, height: 80 }}>
							...
							{editId && (
								<button type="button" onClick={() => removeExistingImage(url, editId)} className="btn btn-danger btn-sm position-absolute top-0 end-0 p-0 lh-1" style={{ width: 20, height: 20, fontSize: 12 }}>×</button>
							)}
						</div>
					))}
				</div>
					...
					<motion.div initial="hidden" animate="visible" variants={fadeInUp} className="card border-0 shadow-lg p-4 p-md-5 backdrop-blur rounded-4">
						<form onSubmit={handlePublish}>
							<AnimatePresence>
								...
								{/* Step 1 */}
								<motion.div key="step1" className={step !== 1 ? "d-none" : ""} initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -20, opacity: 0 }}>
									<h4 className="fw-bold mb-4">Step 1: Event Basics</h4>
									<div className="mb-3">
										<label className="form-label fw-semibold">Event Title</label>
										<input name="title" type="text" className="form-control form-control-lg rounded-3 shadow-sm border-light-subtle" placeholder="e.g. Tech Conference 2026" value={formData.title} onChange={handleChange} />
									</div>
									<div className="row">
										<div className="col-md-6 mb-3">
											<label className="form-label fw-semibold">Category</label>
											<select name="category" className="form-select form-control-lg rounded-3 shadow-sm" value={formData.category} onChange={handleChange}>
									...
									<div className="mb-3">
										...
										<textarea name="description" className="form-control rounded-3 shadow-sm" value={formData.description} onChange={handleChange} rows={4} placeholder="Describe your event..."></textarea>
									</div>
								</motion.div>

								{/* Step 2 */}
								<motion.div key="step2" className={step !== 2 ? "d-none" : ""} initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -20, opacity: 0 }}>
									<h4 className="fw-bold mb-4">Step 2: Logistics &amp; Features</h4>
									<div className="row mb-3">
										<div className="col-md-6">
											<label className="form-label fw-semibold">Date</label>
											<input type="date" name="date" value={formData.date} onChange={handleChange} min={todayStr} className="form-control form-control-lg rounded-3 shadow-sm" />
										</div>
										<div className="col-md-6">
											<label className="form-label fw-semibold">Location</label>
											<input type="text" name="location" value={formData.location} onChange={handleChange} className="form-control form-control-lg rounded-3 shadow-sm" placeholder="Venue or Link" />
										</div>
									</div>
								...
								{/* Step 3 */}
								<motion.div key="step3" className={step !== 3 ? "d-none" : ""} initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -20, opacity: 0 }}>
									...
									<div className="mb-4">
										...
										<div className="btn-group w-100 shadow-sm" role="group">
											<input type="radio" className="btn-check" name="free" id="free" checked={isFree} onChange={() => { setIsFree(true); setFormData((p) => ({ ...p, price: 0 })); }} />
											<label className="btn btn-outline-primary py-2 fw-bold" htmlFor="free">Free Event</label>
											<input type="radio" className="btn-check" name="free" id="paid" checked={!isFree} onChange={() => setIsFree(false)} />
									...
									<div className="row mb-4">
										<AnimatePresence>
											{!isFree && (
												<motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="col-md-6 mb-3 overflow-hidden">
													...
													<div className="input-group">
														...
														<input type="number" id="price" name="price" value={formData.price} onChange={handleChange} className="form-control form-control-lg rounded-end-3 border-start-0 shadow-sm" placeholder="0.00" />
													</div>
												</motion.div>
											)}
										</AnimatePresence>
										<div className={isFree ? "col-md-12" : "col-md-6"}>
											...
											<input type="number" name="capacity" value={formData.capacity} onChange={handleChange} className="form-control form-control-lg rounded-3 shadow-sm" placeholder="e.g. 500" />
										</div>
									</div>
								...
								{/* Step 4 */}
								<motion.div key="step4" className={step !== 4 ? "d-none" : ""} initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -20, opacity: 0 }}>
									<h4 className="fw-bold mb-4">Step 4: Visibility &amp; Team Settings</h4>
									<div className="mb-4">
										<label className="form-label fw-semibold">Visibility</label>
										<select name="visibility" className="form-select form-control-lg rounded-3 shadow-sm" value={formData.visibility} onChange={(e) => { if (e.target.value === "PRIVATE" && isFreeTier) return; handleChange(e); }}>
											<option value="PUBLIC">🌍 Public — Listed in search results</option>
											<option value="UNLISTED">🔗 Unlisted — Only accessible via direct link</option>
											<option value="PRIVATE" disabled={isFreeTier}>🔒 Private — Only confirmed registrants can view{isFreeTier ? " (PRO only)" : ""}</option>
										</select>
										{isFreeTier && <div className="form-text text-warning mt-1">🔒 Private visibility is a PRO feature. <a href="/pricing" className="text-warning fw-bold">Upgrade to PRO</a></div>}
										{formData.visibility === "PRIVATE" && !isFreeTier && <div className="form-text text-info mt-1">Private events skip admin review and go live immediately.</div>}
									</div>
									<hr className="opacity-10 my-4" />
									<div className="p-4 bg-body-tertiary rounded-4 border border-primary border-opacity-10">
										<div className="form-check form-switch mb-1">
											<input className="form-check-input" type="checkbox" id="isTeamEvent" name="isTeamEvent" checked={formData.isTeamEvent} disabled={isFreeTier} onChange={handleChange} />
												...
												<motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
													<div className="row mt-3 g-3">
														<div className="col-md-4">
															...
															<input type="number" name="minTeamSize" value={formData.minTeamSize} onChange={handleChange} className="form-control rounded-3" placeholder="e.g. 2" min={1} />
														</div>
														<div className="col-md-4">
															<label className="form-label fw-semibold small">Max Team Size</label>
															<input type="number" name="maxTeamSize" value={formData.maxTeamSize} onChange={handleChange} className="form-control rounded-3" placeholder="e.g. 5" min={1} />
														</div>
														<div className="col-md-4">
															<label className="form-label fw-semibold small">Capacity Mode</label>
															<select name="teamCapacityMode" className="form-select rounded-3" value={formData.teamCapacityMode} onChange={handleChange}>
							...
							<div className="d-flex justify-content-between mt-5 pt-3 border-top">
								...
								<div className="d-flex gap-2">
									<button type="button" className="btn btn-outline-secondary rounded-pill px-4 fw-bold" onClick={handleSaveDraft} disabled={submitting || uploading || !isStepValid()}>
									...
									{step < TOTAL_STEPS ? (
										...
										<motion.button whileTap={isStepValid() ? { scale: 0.95 } : {}} type="submit" className="btn btn-success px-5 rounded-pill shadow fw-bold" disabled={!isStepValid() || submitting || uploading}>
											{submitting || uploading
												? <><span className="spinner-border spinner-border-sm me-2" role="status" />{uploading ? "Uploading..." : "Publishing..."}</>
												: " Publish Event"
											}
										</motion.button>
									)}
